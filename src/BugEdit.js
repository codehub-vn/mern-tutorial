  ...
  render: function() {
    var success = (
      <Alert bsStyle="success" onDismiss={this.dismissSuccess} dismissAfter={5000}>
        Bug được lưu lại thành công.
      </Alert>
    );
    return (
      <div style={{maxWidth: 600}}>
        <Panel header={"Chỉnh sửa bug: " + this.props.params.id}>
          <form onSubmit={this.submit}>
            <Input type="select" label="Mức Độ Ưu Tiên"
              value={this.state.priority} onChange={this.onChangePriority}>
              <option value="P1">P1</option>
              <option value="P2">P2</option>
              <option value="P3">P3</option>
            </Input>
            <Input type="select" label="Trạng Thái" value={this.state.status} onChange={this.onChangeStatus}>
              <option>New</option>
              <option>Open</option>
            ...
            <ButtonToolbar>
              <Button type="submit" bsStyle="primary">Cập Nhật</Button>
              <Link className="btn btn-link" to="/bugs">Quay Lại</Link>
            </ButtonToolbar>
          </form>
        </Panel>
